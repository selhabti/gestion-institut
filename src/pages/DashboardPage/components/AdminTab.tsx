...
export const AdminTab = ({ user, shareMode, members, onRestoreMember, archivedMembers, loadingArchived, onLoadArchived }: AdminTabProps) => {
  ...
  }, [adminSubTab, archivedMembers, onLoadArchived]);
    ...
    >
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Élèves désactivés mais dont les données de présence sont conservées
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loadingArchived ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-slate-500">
                    ...
                    <th className="pb-3 font-medium">Archivé le</th>
                ...
                <tbody>
                  {(archivedMembers ?? []).map((m) => (
                    <tr key={m.id} className="border-b last:border-0 hover:bg-slate-50">
                      ...
                      <td className="py-3">{m.firstName}</td>
                      <td className="py-3">
                        <Badge variant="outline">{m.group}</Badge>
                      </td>
                      <td className="py-3 text-slate-500">
                        {m.deleted_at ? new Date(m.deleted_at).toLocaleDateString("fr-FR") : "—"}
                      </td>
                      <td className="py-3">
                        <Button
                          size="sm"
